...
/**
 * observations 
 * pdf output is buggy - tried buffer to string etc, but pdf content invalid error occured
 */
const fs = require('fs')
...
async function createPdf(firstPage) {
  ...
  const pdfBuffer = await converter.mkCompoundPdf([
    ...
    {
      pdfContent: fs.readFileSync('restpage.html').toString(),
      margin,
      pageSize: pdf.pageSizeInMM.A4.portrait,
    }
  ])
...
try {
  const pdfContent = createPdf(firstPage)
  const content = pdfContent.toString()
  fs.writeFileSync('result.pdf', content, 'utf-8')
}catch(error) {
  throw(error)
}
// var options = {
